...
/**
 * Chrome MV3 service workers are evicted between `onBeforeSendHeaders` and
 * `downloads.onCreated`, so the cache lives in `storage.session` with the
 * in-memory map as the fast path.
 */
...
/**
 * Chrome's service worker can sleep; a `downloads.onCreated` callback must be
 * able to read the cache synchronously-ish, so callers await this first.
 *
 * @param {string} url
...
/** Drop expired entries and flush the snapshot to storage.session. */
/** @param {number} now */
...
/** Debounced snapshot write; service worker lifetime is short and this is hot. */
